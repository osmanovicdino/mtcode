...
void set_potential_bundle_tetrahedral(Condensate &a, vector1<double> str, vector1<double> dis, vector1<double> ang)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
        {
            KernFrenkelOnePatch2 *pot1 = new KernFrenkelOnePatch2(v(i, 0), v(i, 1), v(i, 2), v(j, 0), v(j, 1), v(j, 2), str[iter], dis[iter], ang[iter], 0.75);
