...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < 3; i++) // caps
    {
        for (int j = 0; j < 2; j++)
        {
            ...
            {
                params(iter, 0) = deltaG; //bind to one end, blocking further growth
                params(iter, 1) = range;
                params(iter, 2) = angle;
            }
            iter++;
    ...
    A.run_with_real_surface_add_particles(300000000, 10000, B, prod, c1, stringbase);
