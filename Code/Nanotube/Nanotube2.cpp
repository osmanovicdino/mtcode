...
void NanotubeAssembly::run_no_patchy(int runtime, int every, ShellProperties &myshell, double prod, double ks, double ks2, string strbase = "")
{
    ...
    for (int k = 0; k < NpolB; k++)
    {
        ...
        if (overlaptest)
            k--; // if an overlap is found, try again
    }
    ...
    for (int i = 0; i < runtime; i++)
    {
        ...
        if (i % every == 0 && i > 0)
        {
            ...
            for (int line1 = 0; line1 < allpolymers.size(); line1++)
            {
                int lin = allpolymers[line1].size() - 1;
                for (int line2 = 0; line2 < lin; line2++)
                {
                    myfile3 << allpolymers[line1][line2] << ",";
                }

                myfile3 << allpolymers[line1][lin] << endl;
            }
...
void NanotubeAssembly::run_no_patchy_noshell(int runtime, int every, double prod, double ks, double ks2, string strbase = "")
{
    ...
    double rmax = 16.;
    spherical_confinement_3D conf(rmax,1.0,ll/2.);
    ...
    for (int i = 0; i < runtime; i++)
    {
        ...
        if (i > 0 && i % 20 == 0)
        {
            ...
            ovf:
                ...
                else if (SQR(x3[0] - ll / 2.) + SQR(x3[1] - ll / 2.) + SQR(x3[2] - ll / 2.) > SQR(rmax - 1))
                {
                    //do not add if it is out of bounds
                }
                else
                {
                    int ij = rand() % indices_to_add.size();
...
void NanotubeAssembly::run_no_patchy_continue(int runtime, int every, int starting_num, ShellProperties &myshell, double prod, double ks, double ks2, matrix<double> &olddat, vector1<int> &oldint, vector<vector<int>> &oldallpolymers, string strbase = "")
{
    ...
    vector<vector<int>> allpolymers(oldallpolymers);



    for(int i =  0 ; i < allpolymers.size() ; i++) {


        for(int j = 0  ; j < allpolymers[i].size()-1 ; j++) {
        // allpolymers[i].push_back(totnp+ i*3+j);
        if(j < allpolymers[i].size()-2){
        mdtriplet mdt(allpolymers[i][j], allpolymers[i][j+1],allpolymers[i][j+2]);
        triplets.push_back(mdt);
        }
        mdpair mdp1(allpolymers[i][j], allpolymers[i][j+1]);
        // mdpair mdp2(totnp + i * 3 + 1, totnp + i * 3 + 2);
...
void NanotubeAssembly::run_with_real_surface_add_particles_steric(int runtime, int every, ShellProperties &myshell, double prod, WeiM &c1, string strbase = "")
{
    ...
    for(int i = totnp ; i < NN ; i++) { //we add all the particles
        indices_everything.push_back(i);
    }
    indices_patchy.reserve(Nx);
    ...
    for (int i = 0; i < runtime; i++)
    {
        ...
        if (i > 0 && i % 20 == 0)
        {
            ...
            if(ra < prod) {
                ...
                double result = std::reduce(indices_weights.begin(), indices_weights.end());

                double randv = result * ((double)rand() / (double)(RAND_MAX));

                double w = 0.0;
                int index_which = 0;
                for (int jj = 0; jj < indices_to_add.size(); jj++)
                {
                    w += indices_weights[jj];
                    if (w > randv)
                    {
                        index_which = jj;
                        break;
                ...
                int fi = indices_to_add[index_which];

                remove_at(indices_weights, index_which);
                remove_at(indices_to_add, index_which);

                indices_patchy.push_back(fi); //activate the patchy particle
