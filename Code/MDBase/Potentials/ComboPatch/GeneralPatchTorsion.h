#ifndef GENERALPATCHTENSION_H
#define GENERALPATCHTENSION_H

struct GeneralPatchTorsion : ComboPatch {
    ...
    GeneralPatchTorsion(vector1<int>no_patches_per_typee, vector1<int> num_per_typee, matrix<double>&, matrix<double>& , vector1<int> &torsionn, bool);
    GeneralPatchTorsion(const GeneralPatchTorsion&);
    ...
    void CreateFiles();
    GeneralPatchTorsion *clone() const
    {
        return new GeneralPatchTorsion(*this);
    }
    //the parameter matrix we have must be
};

#include "GeneralPatchTorsion.cpp"
