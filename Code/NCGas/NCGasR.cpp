...
NCGasR::NCGasR(double ll, int totalN) {
	...
	for(int i = 0 ; i < totalN ; i++) {
		res(i,0)=pi*((double) rand() / (RAND_MAX));
		res(i,1)=2*pi*((double) rand() / (RAND_MAX));
	}
...
//	f.add_interaction_all(nswca);
	...
	WCAPotential nswca(10.0, sigma, eqeps);	
	// int sweeps = 5;
	// double max_step = 0.2;
...
matrix<double> NCGasR::PositionForcesDueToAngles_mips() {
	int totalN = obj->getN();
...
void NCGasR::UpdateAngles(matrix<double> &angularforces) {
	...
	for(int i = 0 ; i < totalN ; i++) { //this would choose a random new orientation as a perturbation of the previous one
		...
		double t1 = newtheta + fac*dt*angularforces(i,0);
		if(t1<0) t1+=2*pi;
		else if(t1>2*pi) t1-=2*pi;
		else{}

		double t2 = newphi + fac*dt*angularforces(i,1);;

		if(t2<0) t2+=2*pi;
		else if(t2>2*pi) t2-=2*pi;
		else{}
		(*orient)(i,0) = t1;
		(*orient)(i,1) = t2;
	}
...
void NCGasR::run(int runtime)
{
	...
	if(i%15==0) {
		...
		matrix<double> F2((this->PositionForcesDueToAngles_mips()));
